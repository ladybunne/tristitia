...
async function loadRuns(client) {
	...
	// AAAAAAAAH THIS ISN'T WORKING PROPERLY
	// weird stuff is happening where loading cancelledRuns OVERWRITES futureRuns.
	// that's cursed and bad and I need to sleep before fixing this.

	await fs.readFile(config.futureRunsFilename)
		.then((data) => futureRuns = JSON.parse(data).map(run =>
			Object.assign(BARun.prototype, run)))
		.catch((error) => console.error(error));

	await fs.readFile(config.pastRunsFilename)
		.then((data) => pastRuns = JSON.parse(data).map(run =>
			Object.assign(BARun.prototype, run)))
		.catch((error) => console.error(error));

	await fs.readFile(config.cancelledRunsFilename)
		.then((data) => cancelledRuns = JSON.parse(data).map(run =>
			Object.assign(BARun.prototype, run)))
		.catch((error) => console.error(error));
